(function(rootObject) {
      ...
      game.allObjects().forEach(function(otherMovObj) {
        if (movObj !== otherMovObj) {
              ...
              if (game.isNewHighScore()) {
                var hs = new Asteroids.HighScore({
                  score: game.score,
                  bulletsFired: game.bulletsFired,
                  level: game.level
                });
                hs.show();
              }
  ...
  Game.prototype.isNewHighScore = function () {
    var highScores = this.menu.highScores;
    return highScores.length == 0 || this.score > highScores[highScores.length - 1].score;
  };
  ...
  Game.prototype.deathScreen = function () {
    ...
    var text = "Game Over";
    var accuracy = Math.round(((this.score/12) / this.bulletsFired) * 100);
