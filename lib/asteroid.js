(function (rootObject){
  ...
  Asteroid.prototype.explode = function (game) {
    var babies = [];
    var parent = this;
    var offsetFactor = 35;
    var offsets = [
      [offsetFactor, offsetFactor], [offsetFactor, -offsetFactor],
      [-offsetFactor, -offsetFactor], [-offsetFactor, offsetFactor]
    ];
    _.each(offsets, function(set) {
      var newVel = [1,1];
      newVel[0] = set[0] > 0 ? 1 : -1;
      newVel[1] = set[1] > 0 ? 1 : -1;
      var littleAsteroid = new Asteroids.Asteroid({
        radius: 10,
        origin: {
          x: parent.origin.x + set[0],
          y: parent.origin.y + set[1]
        },
        vel: newVel
      });
      babies.push(littleAsteroid);
    });
    return babies;
  };
